...
while True:
    if len(SID) != 3 or not SID.isdigit():
        ...
        gender = question('Gender (F/M): ', (qpos[0], qpos[1] + int(fontSize*3.33)),size=(int(fontSize*10), int(fontSize*1.67)))
...
DataMain.writerow(
    ['SID', 'gender', 'age', 'trial', 'step', 'submitted', 'instanceID', 'itemID', 'weight', 'value', 'selected',
     'totalWeight', 'totalValue', 'selectStatus', 'difficulty_estimate', 'performance_estimate', 'weights', 'values',
     'capacity', 'group', 'solution_val', 'solution_comb', 'sahniK', 'n_item', 'normalized_capacity',
     'normalized_profit', 'ratio'])
DataFileMain.flush()
...
for i in range(0, len(instances)):
    ...
    while time.time() - instanceOnset < 10:
        checkExit()
            ...
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                ...
                if all(click_list == 0) and submitted == 0:
                    ...
                    dataToWrite = [SID, gender,age, i, step, submitted, oriIdx[i], clicked_bag_idx, w[clicked_bag_idx],
                                   ...
                                   difficulty_estimate, performance_estimate]
    ...
    while True:
        screen.fill(colors.grey)
        difficulty_q = 'Having done this instance, please rate the difficulty of it on a 1-10 scale (10 being most difficult):'
        difficulty_experienced = question(difficulty_q, (leftPad, screenHeight // 3),
                                          size=(q_width, int(fontSize*3.33)))
        if not difficulty_experienced.isnumeric() or int(difficulty_experienced) > 10 or int(difficulty_experienced) < 1:
            question(difficulty_q + '\nAnswer should be an integer between 1 and 10! Press Enter to try again.',
                     (leftPad, screenHeight // 3), size=(q_width, int(fontSize*5)),
                     allowanswer=False)
        else:
